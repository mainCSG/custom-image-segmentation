...
def PolyArea(x: np.array, y: np.array) -> float:
    return 0.5 * np.abs(
        np.dot(
            x,
            np.roll(y,1)
...
def augment_image(filepath: str, annotations: str, augmentations: A.BaseCompose) -> tuple:
    ...
    # Convert masks back to the appropriate polygon format and 
    # make sure they have the same label!
    augmented_polygons = []
...
def main(training_data_dir: Path, num_exp_aug: float, num_sim_aug: int) -> None:
    ...
    for image_filepath in training_data_dir.glob("*.jpg"):
        ...
        try:
            image_annotations = annotations[image_filepath.name]
        except KeyError:
            print(f"No annotations to augment for file {image_filepath.name}!")
            continue 
    ...
    print(f"Saving AUGMENTATED annotations for directory {training_data_dir}")
    annotations_json_file = training_data_dir / "annotations.json"
